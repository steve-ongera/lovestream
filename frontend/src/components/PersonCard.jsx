    ...
    <Link to={`/profile/${person.slug}`} className="ls-person-card">
      ...
      <div className="ls-person-card__body">
        <h3 className="ls-person-card__name">
          ...
          {person.age && <span style={{ fontWeight: 400, fontSize: 16 }}>, {person.age}</span>}
        </h3>

        <p className="ls-person-card__meta">
          {[person.city, person.country].filter(Boolean).join(", ") || "Unknown location"}
        </p>

        {/* Physical attributes */}
        <div className="ls-person-card__attrs">
          {person.height_cm && (
            <span className="ls-pill"><i className="bi bi-arrows-vertical" /> {person.height_cm}cm</span>
          )}
          {person.skin_texture && (
            <span className="ls-pill">{person.skin_texture}</span>
          )}
          {person.relationship_goal && (
            <span className="ls-pill"><i className="bi bi-heart-arrow" /> {person.relationship_goal}</span>
          )}
          {person.is_premium && (
            <span className="ls-pill"><i className="bi bi-gem" /> Premium</span>
          )}
        </div>

        {/* Actions — stop propagation so Link doesn't fire */}
        <div className="ls-person-card__actions" onClick={(e) => e.preventDefault()}>
          <button
            className={`ls-btn ls-btn--sm ${matched ? "ls-btn--ghost" : "ls-btn--primary"}`}
            onClick={handleMatch}
