  ...
  return (
    <header className="ls-navbar">
      <div className="ls-navbar__inner">
        ...
        {/* ── Hamburger (mobile) ─────────────────────────────────────── */}
        <button
          className="ls-navbar__hamburger"
          onClick={onMenuClick}
        ...
        <Link to="/" className="ls-navbar__logo">
          ...
          <span className="ls-navbar__logo-word">
        ...
        <form className="ls-navbar__search" onSubmit={handleSearch}>
          ...
          {search && (
            <button
              type="button"
              className="ls-navbar__search-clear"
              onClick={() => setSearch("")}
        ...
        <div className="ls-navbar__controls">
          ...
          <Dropdown
            ...
            trigger={
              <button className="ls-navbar__ctrl-btn" title="Language">
                <span>{lang.flag}</span>
                <span className="ls-navbar__ctrl-label">
          ...
          >
            <p className="ls-dropdown__section-label">Language</p>
            {LANGUAGES.map((l) => (
              <button
                key={l.code}
                className="ls-dropdown__item"
                onClick={() => setLang(l)}
              >
                <span>{l.flag}</span>
                {l.label}
          ...
          <Dropdown
            ...
            trigger={
              <button className="ls-navbar__ctrl-btn" title="Filter by gender">
                ...
                <span className="ls-navbar__ctrl-label ls-hide-sm">
                  {gender.label}
                </span>
                <i className="bi bi-chevron-down" style={{ fontSize: 10 }} />
              </button>
          ...
          >
            <p className="ls-dropdown__section-label">Browse</p>
            {GENDERS.map((g) => (
              <button
                key={g.value}
                className="ls-dropdown__item"
          ...
          <Link
            to="/premium"
            className="ls-navbar__premium ls-hide-sm"
          >
          ...
          <Link
            to="/settings"
            className="ls-navbar__ctrl-btn ls-hide-sm"
            title="Settings"
          >
          ...
          {user && (
            <button
              className="ls-navbar__ctrl-btn ls-notif-btn"
              title="Notifications"
            >
          ...
          {user ? (
            <Dropdown
              align="right"
              trigger={
                /* Use the ls-avatar system so border, sizing & online-dot
                   all come from the design-system rules in main.css        */
                <button
                  className={`ls-avatar ls-avatar--sm ls-avatar--red-ring${
                    user.is_online ? " ls-avatar--online" : ""
                  }`}
                  title="My account"
            ...
            >
              ...
              <button
                className="ls-dropdown__item ls-dropdown__item--danger"
                onClick={logout}
              >
